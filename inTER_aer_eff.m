...
for c = 1:5
    ...
    for itype = 1:length(indvar_combined)
        ...
        text(xmax_v*1.05, y_hat(xmax_i)*1.05,sprintf('%.4f', rsq),...
        ...
        if itype == 1
            ...
            %         title(['INTER-cloud \epsilon vs ' title_combined{itype}])
        else
            set(gca,'YTick',[])
            %         title(['vs ' title_combined{itype}])
        end
...
sgt = sgtitle('Inter-cloud correlation of relative dispersion with different environmental variables - all campaigns');
sgt.FontSize = 22;
sgt.FontWeight = 'bold';
...
% set(gca,'Xcolor',axis_color)
% set(gca,'Ycolor',axis_color)
saveas(gcf,'plots/INTER-cloud_3.png')
